...
from keras.layers import Dense, Input, Dropout, Embedding, LSTM,Bidirectional, Multiply,Permute,GRU
from keras.optimizers import RMSprop, Adam, Nadam
from keras.preprocessing import sequence

from keras.layers import Conv1D, Flatten, Activation, SpatialDropout1D,add
...
#dataset
path = './data/16-20Q1/dataset_ok.csv'
...
def ResBlock(x, filters, kernel_size, dilation_rate):
    r = Conv1D(filters, kernel_size, padding='same', dilation_rate=dilation_rate, activation='relu')(
        x)  # first convolution
    r = Conv1D(filters, kernel_size, padding='same',
               dilation_rate=dilation_rate)(r)  # Second convolution

    if x.shape[-1] == filters:
        shortcut = x
    else:
        shortcut = Conv1D(filters, kernel_size, padding='same')(
            x)  	# shortcut (shortcut)
    o = add([r, shortcut])
    # Activation function
    o = Activation('relu')(o)
...
def TCN(train_x, train_y, valid_x, valid_y, test_x, test_y, classes, epoch):
    ...
    model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=[
                  AUC(name='AUC')])  # categorical_crossentropy mean_squared_error
    # Training model 训练模型
    model.fit(train_x, train_y, batch_size=500, epochs=epoch,
