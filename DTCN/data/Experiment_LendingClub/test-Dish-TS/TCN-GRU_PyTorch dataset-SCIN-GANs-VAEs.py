...
class TCN_DishTS(nn.Module):
    def __init__(self, input_channels, output_classes, dish_ts_module, hidden_size, num_layers):
        ...
        # Add a linear layer to adjust the input size for the GRU layer
        self.linear = nn.Linear(61, 32)
        ...
        self.fc = nn.Linear(hidden_size * 32, output_classes)
    ...
    def forward(self, x):
        ...
        # Pass the output through the linear layer to adjust the input size for the GRU layer
        x = self.linear(x)
...
hidden_size = 64
num_layers = 1
model = TCN_DishTS(input_channels=num_channels, output_classes=1, dish_ts_module=dish_ts_module, hidden_size=hidden_size, num_layers=num_layers)
